...
//
// On first load the characters fly in from the left and resolve into the picture.
...
export function Portrait({ ground = GROUND, ink = INK, rows: rowCount = ROWS, ramp: rampText = RAMP, sharpen = SHARPEN, soften = 0, intro = true, onReady }: Props = {}) {
  ...
  useEffect(() => {
    ...
    let spritesBig: HTMLCanvasElement[] = [];
    ...
    function drawFlying(x: number, y: number, ch: number, alpha: number) {
      if (ch <= 0 || alpha <= 0.01) return;
      ctx.globalAlpha = alpha;
      ctx.drawImage(spritesBig[ch], Math.round((x - cellW * 0.175) * dpr), Math.round((y - cellH * 0.175) * dpr));
    }
    ...
    function detach(c: Cell) {
      ...
      const speed = (H / 900) * (50 + 190 * c.f) * (0.7 + Math.random() * 0.6);
      c.vx = -speed;
      c.vy = (Math.random() - 0.5) * 18 * (H / 900);
      c.x = c.hx;
      c.y = c.hy;
      const t = tone[c.i];
      c.chNow = charFor(t);
    ...
    function build(family: string) {
      ...
      spritesBig = makeSpriteSet(family, 1.35);
    ...
    function stepField(dt: number) {
      ...
      for (const c of loose) {
        if (c.phase === 0) {
          ...
          c.t += dt;
          if (c.t >= c.life || c.x < -cellW || c.y < -cellH || c.y > H) {
            c.phase = 2;
            c.t = 0;
            continue;
          }
          ...
          const sy = c.y * 0.0045;
          const e = 0.35;
          const curlX = (noise(sx + nt, sy + e) - noise(sx + nt, sy - e)) / (2 * e);
          const curlY = -(noise(sx + e + nt, sy) - noise(sx - e + nt, sy)) / (2 * e);
          const drift = H / 900;
          c.x += (c.vx + curlX * 25 * drift) * dt;
          c.y += (c.vy + curlY * 55 * drift) * dt;
          c.mutateAt -= dt;
          if (c.mutateAt <= 0) {
    ...
    function render() {
      ...
      for (const c of cells) {
        ...
        if (it < INTRO_END) {
          ...
          drawCell(x + c.ix * (1 - e), y + c.iy * (1 - e), flying, alpha * smooth(p * 1.6));
          continue;
        }
      ...
      for (const c of loose) {
        ...
        drawCell(c.x - c.vx * 0.14, c.y - c.vy * 0.14, Math.max(1, c.chNow - 1), a * 0.16);
        drawCell(c.x - c.vx * 0.07, c.y - c.vy * 0.07, c.chNow, a * 0.4);
        drawFlying(c.x, c.y, c.chNow, a);
      }
      ctx.globalAlpha = 1;
    }
